...
public class JSmoothModelPersistency {
	public static JSmoothModelBean load(File fin) throws IOException {
		...
		try {
			...
			// System.out.println("Loaded jobj " + jobj + " = " +
			// jobj.getJarLocation());
			if ((jobj.getJarLocation() != null)
					&& (jobj.getJarLocation().length() > 0)) {
				jobj.setEmbeddedJar(true);
				// System.out.println("Set embeddedjar to " +
				// jobj.getEmbeddedJar());
			}
			...
			try {
				...
				if ((xobj.getJarLocation() != null)
						&& (xobj.getJarLocation().length() > 0))
	...
	static public File makePathRelativeIfPossible(File root, File f) {
		...
		while (cur != null) {
			...
			// lame hack, because getName() returns "" when the file is a drive
			// (like c: or z:)
			if (n.equals(""))
	...
	static public void main(String[] args) {
		...
		System.out.println("Rel root, t1: "
				+ makePathRelativeIfPossible(root, t1));
		System.out.println("Rel root, t2: "
				+ makePathRelativeIfPossible(root, t2));
		System.out.println("Rel root, t3: "
				+ makePathRelativeIfPossible(root, t3));
		System.out.println("Rel root, t4: "
				+ makePathRelativeIfPossible(root, t4));
