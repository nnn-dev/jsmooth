...
public class SkeletonPropertiesEditor extends Editor implements
		JSmoothModelBean.SkeletonChangedListener {
	/**
	 * 
	 */
	private static final long serialVersionUID = 1548695197934766215L;
	...
	public void rebuildProperties() {
		...
		for (int i = 0; i < sprops.length; i++) {
			...
			if (sprops[i].getType().equalsIgnoreCase(
					SkeletonProperty.TYPE_STRING)) {
				spe = new StringEditor();
			} else if (sprops[i].getType().equalsIgnoreCase(
					SkeletonProperty.TYPE_TEXTAREA)) {
				spe = new TextAreaEditor();
			} else if (sprops[i].getType().equalsIgnoreCase(
					SkeletonProperty.TYPE_BOOLEAN)) {
				spe = new CheckBoxEditor();
			} else if (sprops[i].getType().equalsIgnoreCase(
					SkeletonProperty.TYPE_AUTODOWNLOADURL)) {
			...
			if (spe.labelAtLeft()) {
				...
				m_panel.add("tab",
						new HelpButton(Main.local(sprops[i].getDescription())));
				m_panel.add("tab hfill", spe.getGUI());
			} else {
				m_panel.add("br right", spe.getGUI());
				m_panel.add("tab",
						new HelpButton(Main.local(sprops[i].getDescription())));
				m_panel.add("tab hfill",
						new JLabel(Main.local(sprops[i].getLabel())));
			}
		}
	...
	public void dataChanged() {
		// System.out.println("========================================================");
		// System.out.println("SkeletonPropertiesEditor: data changed, " +
		...
		if ((m_model != null)
				&& (m_model.getSkeletonName() != null)
				&& (!m_model.getSkeletonName().equalsIgnoreCase(
						m_currentSkelName))) {
		...
		// System.out.println("jsprop is null ? " + jsprop + " / " +
		// ((jsprop!=null)?jsprop.length:-1));
		if (jsprop != null) {
			for (Enumeration e = m_editors.elements(); e.hasMoreElements();) {
				SkelPropEditor spe = (SkelPropEditor) e.nextElement();
				JSmoothModelBean.Property p = getPropertyInstance(spe
						.getIdName());
				if (p != null)
					spe.valueChanged(p.getValue());
			}
		} else { // if no properties are defined for this model, we use the
					// default values

			SkeletonBean skel = Main.SKELETONS.getSkeleton(m_model
					.getSkeletonName());
			SkeletonProperty[] sprops = null;
			if (skel != null)
		...
		// System.out.println("DONE NOTHING! " +m_currentSkelName + "/" +
		// m_model.getSkeletonName());
	}
	...
	public void updateModel() {
		if (m_skel != null) {
			JSmoothModelBean.Property[] props = new JSmoothModelBean.Property[m_editors
					.size()];
			int index = 0;
			for (Enumeration e = m_editors.elements(); e.hasMoreElements();) {
				SkelPropEditor spe = (SkelPropEditor) e.nextElement();
				// System.out.println("IMODEL property " + spe + "/" +
				// spe.getIdName() + "=" + spe.get());
		...
		// JSmoothModelBean.Property[] props = new
		// JSmoothModelBean.Property[sp.length];
