...
public class MasterPanel extends JPanel {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 7076464919250294032L;
	...
	private Object[] m_jsInfo = { new net.charabia.jsmoothgen.application.gui.editors.JSmoothInfo() };
	...
	public MasterPanel() {
		...
		addAction("Application", "/icons/stock_form-image-control.png",
				m_appElements);
		addAction("JVM Selection", "/icons/stock_search.png", m_jvmSelElements);
		addAction("JVM Configuration", "/icons/stock_form-properties.png",
				m_jvmCfgElements);
	...
	private void addAction(final String name, String iconloc, final Object[] els) {
		final Action a = new AbstractAction(name, new ImageIcon(getClass()
				.getResource(iconloc))) {
			/**
					 * 
					 */
					private static final long serialVersionUID = -3741860894556363962L;
		...
		for (int i = 0; i < els.length; i++) {
			if (els[i] instanceof Editor) {
				...
				if (pgroup == null) {
					...
					javax.swing.border.TitledBorder title = BorderFactory
							.createTitledBorder(
									BorderFactory
				...
				if (ed.needsFullSpace()) {
					...
					javax.swing.border.TitledBorder title = BorderFactory
							.createTitledBorder(
									BorderFactory
											.createEtchedBorder(javax.swing.border.EtchedBorder.LOWERED),
									getLocaleText(ed.getLabel()));
					pgroup.setBorder(title);
					pgroup.add("left vtop",
							new HelpButton(getLocaleText(ed.getDescription())));
					...
					if (ed.useDescription()) {
						pgroup.add("br left",
								new JLabel(getLocaleText(ed.getLabel())));
						pgroup.add(
								"tab",
								new HelpButton(getLocaleText(ed
										.getDescription())));
	...
	private void attachAll() {
		// System.out.println("Attaching all with " + m_modelLocation + ": " +
		// m_model);
		for (Iterator i = m_displayedElements.iterator(); i.hasNext();) {
			Editor ed = (Editor) i.next();
	...
	public ExeCompiler.CompilerRunner getCompiler() {
		...
		try {
			...
			ExeCompiler.CompilerRunner runner = compiler.getRunnable(skelroot,
					skel, basedir, m_model, out);
			return runner;
		} catch (Exception exc) {
			exc.printStackTrace();
