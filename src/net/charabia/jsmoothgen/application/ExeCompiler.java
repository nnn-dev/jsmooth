...
public class ExeCompiler {
	...
	public class CompilerRunner implements Runnable {
		...
		public CompilerRunner(File skelroot, SkeletonBean skel, File basedir,
				JSmoothModelBean data, File out) {
			m_skelroot = skelroot;
			m_skel = skel;
			m_data = data;
			m_out = out;
	...
	public ExeCompiler.CompilerRunner getRunnable(File skelroot,
			SkeletonBean skel, File basedir, JSmoothModelBean data, File out) {
		return new CompilerRunner(skelroot, skel, basedir, data, out);
	}

	public void compileAsync(File skelroot, SkeletonBean skel, File basedir,
			JSmoothModelBean data, File out) {
		Thread t = new Thread(new CompilerRunner(skelroot, skel, basedir, data,
				out));
		t.start();
	}

	public boolean compile(File skelroot, SkeletonBean skel, File basedir,
			JSmoothModelBean data, File out) throws Exception {
		try {
			...
			if (data.getEmbeddedJar() == true) {
				...
				resb = resdir.replaceResource(skel.getResourceCategory(),
						skel.getResourceJarId(), 1033, jardata);
				if (resb == false) {
			...
			resb = resdir.replaceResource(skel.getResourceCategory(),
					skel.getResourcePropsId(), 1033, propdata);
			...
			if (data.getIconLocation() != null) {
				...
				if (new java.io.File(data.getIconLocation()).isAbsolute())
					...
					iconpath = new java.io.File(basedir, data.getIconLocation())
							.getAbsolutePath();
				...
				if (img != null) {
					net.charabia.jsmoothgen.pe.res.ResIcon resicon = new net.charabia.jsmoothgen.pe.res.ResIcon(
							img);
	...
	public Image[] loadImages(String path) {
		...
		if (path.toUpperCase().endsWith(".ICO")) {
			...
			//
			try {
				java.awt.Image[] images = net.charabia.util.codec.IcoCodec
						.loadImages(f);
	...
	public BufferedImage getQuantizedImage(Image img) {
		...
		int[] palette = net.charabia.util.codec.Quantize.quantizeImage(data,
				255);
		...
		IndexColorModel colmodel = new IndexColorModel(8, palette.length, cmap,
				0, true, 0);
	...
	public Image getScaledImage(String path, int width, int height) {
		...
		Image biggest = null;
		for (int i = 0; (i < orgimages.length) && (selected == null); i++) {
			int size = orgimages[i].getWidth(null)
					* orgimages[i].getHeight(null);
			if (size > maxsize) {
				maxsize = size;
				biggest = orgimages[i];
			}
		...
		if (biggest != null) {
			Image result = biggest.getScaledInstance(32, 32,
