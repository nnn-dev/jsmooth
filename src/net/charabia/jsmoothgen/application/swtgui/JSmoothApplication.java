...
public final class JSmoothApplication {
	public final JSmoothAction ACTION_OPEN = new OpenAction(
			JSmoothApplication.this);
	public final JSmoothAction ACTION_EXIT = new ExitAction(
			JSmoothApplication.this);
	public final JSmoothAction ACTION_SAVE = new SaveProjectAction(
			JSmoothApplication.this);
	public final JSmoothAction ACTION_SAVE_AS = new SaveAsProjectAction(
			JSmoothApplication.this);
	public final JSmoothAction ACTION_NEW = new NewAction(
			JSmoothApplication.this);
	public final JSmoothAction ACTION_COMPILE = new CompileAction(
			JSmoothApplication.this);
	public final JSmoothAction ACTION_RUNEXE = new RunexeAction(
			JSmoothApplication.this);
	public final JSmoothAction ACTION_CONSOLE_CLEAR = new ClearConsoleAction(
			JSmoothApplication.this);
	...
	// JSmooth Pages
	public final JSmoothPage PAGE_SKELETON = new SkeletonPage(
			JSmoothApplication.this);
	public final JSmoothPage PAGE_EXECUTABLE = new ExecutablePage(
			JSmoothApplication.this);
	public final JSmoothPage PAGE_WELCOME = new WelcomePage(
			JSmoothApplication.this);
	public final JSmoothPage PAGE_APPLICATION = new JavaAppPage(
			JSmoothApplication.this);
	public final JSmoothPage PAGE_JVM_OPTIONS = new JVMOptionsPage(
			JSmoothApplication.this);
	public final JSmoothPage[] PAGES = new JSmoothPage[] { PAGE_WELCOME,
			PAGE_SKELETON, PAGE_APPLICATION, PAGE_JVM_OPTIONS, PAGE_EXECUTABLE };
	...
	public void setSkeletonProperty(SkeletonProperty property) {
		System.out.println("[DEBUG] Setting property \"" + property.getIdName()
				+ "\" to value \"" + property.getValue() + "\"");
		JSmoothModelBean.Property[] modelProps = jsmodel
	...
	public SkeletonProperty[] getInititalSkeletonProperties() {
		SkeletonBean skeleton = skeletonList.getSkeleton(jsmodel
				.getSkeletonName());
	...
	private void createConsole(Shell shell) {
		...
		console = new Text(consolearea, SWT.MULTI | SWT.H_SCROLL | SWT.V_SCROLL
				| SWT.READ_ONLY | SWT.BORDER);
		console.setBackground(shell.getDisplay()
	...
	public List getSkeletonList(boolean flush) {
		...
		if (skeletonNames == null)
			fillSkeletonList(skeletonNames = new Vector());
		if (flush) {
			skeletonNames.clear();
	...
	public boolean saveProjectAs(String projectfile) {
		...
		try {
			...
			System.out.println("[ERROR] Failed saving project : "
					+ e.getMessage());
			return false;
		}
		getShell().setText(getProjectName());
	...
	public boolean openProject(String projectfile) {
		...
		try {
			...
			System.out.println("[ERROR] Failed opening project : "
					+ e.getMessage());
			return false;
		}
	...
	public boolean compileProject() {
		...
		try {
			success = compiler.compile(skeletonRoot, skeletonBean, basedir,
					jsmodel, exename);
		} catch (Exception e) {
		...
		protected void layout(Composite cmp, boolean flushCache) {
			...
			for (int i = 0; i < wgs.length; i++) {
				...
				if (menusep == w) {
					...
					p = w.computeSize(SWT.DEFAULT, DIM_CONSOLE_HEIGHT,
							flushCache);
					w.setBounds(clientarea.x, clientarea.height
							- DIM_CONSOLE_HEIGHT, clientarea.width, p.y);
