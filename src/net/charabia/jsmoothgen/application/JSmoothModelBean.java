...
public class JSmoothModelBean {
	...
	public void addSkeletonChangedListener(
			JSmoothModelBean.SkeletonChangedListener l) {
		m_skeletonChangedListener.add(l);
	}

	public void removeSkeletonChangedListener(
			JSmoothModelBean.SkeletonChangedListener l) {
	...
	private void fireSkeletonChanged() {
		for (Iterator i = m_skeletonChangedListener.iterator(); i.hasNext();) {
			JSmoothModelBean.SkeletonChangedListener l = (JSmoothModelBean.SkeletonChangedListener) i
					.next();
			l.skeletonChanged();
		}
	}
	...
	public void setSkeletonProperties(JSmoothModelBean.Property[] props) {
		...
		// System.out.println("SET PROPERTY: " + props[i].getIdName() + "=" +
		// props[i].getValue());
	...
	public String[] normalizePaths(java.io.File filebase, boolean toRelativePath) {
		// System.out.println("Normalize Path " + filebase + " / " +
		// toRelativePath);
		Vector result = new Vector();

		m_iconLocation = checkRelativePath(filebase, m_iconLocation, result,
				"Icon location", toRelativePath);
		m_jarLocation = checkRelativePath(filebase, m_jarLocation, result,
				"Jar location", toRelativePath);
		m_bundledJVM = checkRelativePath(filebase, m_bundledJVM, result,
				"Bundle JVM location", toRelativePath);
		m_executableName = checkRelativePath(filebase, m_executableName,
				result, "Executable location", toRelativePath);

		if (m_executableName != null) {
			File exebase = new File(m_executableName);
			if (exebase.isAbsolute() == false)
				exebase = new File(filebase, exebase.toString())
						.getParentFile();

			// System.out.println("EXE FILEBASE: " + exebase.toString());
			if ((m_currentDirectory != null)
					&& (m_currentDirectory.indexOf("${") >= 0))
				m_currentDirectory = checkRelativePath(exebase,
						m_currentDirectory, result, "Current directory",
						toRelativePath);
		}

		if (m_classPath != null) {
			for (int i = 0; i < m_classPath.length; i++) {
				m_classPath[i] = checkRelativePath(filebase, m_classPath[i],
						result, "Classpath entry (" + i + ")", toRelativePath);
			}
		}
	...
	private String checkRelativePath(java.io.File root, String value,
			java.util.Vector errors, String name, boolean toRelativePath) {
		if (value == null)
			return value;

		if (toRelativePath) {
			File nf = JSmoothModelPersistency.makePathRelativeIfPossible(root,
					new File(value));
			if (nf.isAbsolute()) {
				errors.add(name);
			}
