...
public class PropertiesBuilder {
	 ...
	 * <p>
	 * 
	 * Note that all the paths are converted to be made relative to the basedir
	 * parameter provided. All the paths converted are expected to be relative
	 * to the targetted executable binary (before the conversion is applied,
	 * that is).
	 * 
	 */
	static public String makeProperties(File basedir, JSmoothModelBean obj) {
		...
		if (obj.getMaximumMemoryHeap() > 1) {
			addPair("maxheap", Integer.toString(obj.getMaximumMemoryHeap()),
					out);
		}

		if (obj.getInitialMemoryHeap() > 1) {
			addPair("initialheap",
					Integer.toString(obj.getInitialMemoryHeap()), out);
		}
		...
		if (obj.getBundledJVMPath() != null)
			addPair("bundledvm",
					getRenormalizedPathIfNeeded(obj.getBundledJVMPath(),
							basedir, curdir), out);

		if (obj.getClassPath() != null) {
			String[] relcp = new String[obj.getClassPath().length];
			for (int i = 0; i < relcp.length; i++) {
				relcp[i] = getRenormalizedPathIfNeeded(obj.getClassPath()[i],
						basedir, curdir);
			}
		...
		if (javapairs != null) {
			addPair("javapropertiescount",
					new Integer(javapairs.length).toString(), out);
	...
	/**
	 * Converts a path relative to previousbasedir into a path relative to
	 * newbasedir.
	 */
	static public String getRenormalizedPathIfNeeded(String value,
			File previousbasedir, File newbasedir) {
		...
		File n = JSmoothModelPersistency.makePathRelativeIfPossible(newbasedir,
				abs);
