...
public class PEHeader implements Cloneable {
	...
	public void dump(PrintStream out) {
		...
		out.println("long DebugDirectory_Size=" + DebugDirectory_Size
				+ " // ac");
	...
	public void updateVAAndSize(Vector oldsections, Vector newsections) {
		...
		// System.out.println("New BaseOfCode=" + codebase + " (size=" +
		// codesize + ")");
		...
		// System.out.println("New BaseOfData=" + database + " (size=" +
		// datasize + ")");
		...
		ResourceDirectory_VA = findNewVA(ResourceDirectory_VA, oldsections,
				newsections);
		ExceptionDirectory_Size = findNewSize(ExceptionDirectory_VA,
				oldsections, newsections);
		...
		DebugDirectory_VA = findNewVA(DebugDirectory_VA, oldsections,
				newsections);
		ArchitectureSpecificData_Size = findNewSize(
				ArchitectureSpecificData_VA, oldsections, newsections);
		...
		LoadConfigurationDirectory_VA = findNewVA(
				LoadConfigurationDirectory_VA, oldsections, newsections);
		...
		BoundImportDirectoryinheaders_VA = findNewVA(
				BoundImportDirectoryinheaders_VA, oldsections, newsections);
		ImportAddressTable_Size = findNewSize(ImportAddressTable_VA,
				oldsections, newsections);
		...
		DelayLoadImportDescriptors_VA = findNewVA(
				DelayLoadImportDescriptors_VA, oldsections, newsections);
		COMRuntimedescriptor_Size = findNewSize(COMRuntimedescriptor_VA,
				oldsections, newsections);
		COMRuntimedescriptor_VA = findNewVA(COMRuntimedescriptor_VA,
				oldsections, newsections);
	}
	...
	private long findNewVA(long current, Vector oldsections, Vector newsections) {
		for (int i = 0; i < oldsections.size(); i++) {
			...
			if (sect.VirtualAddress == current) {
				...
				// System.out.println("Translation VA found for " + current +
				// " = " + i + " (" +newsect.VirtualAddress + ")=" +
				// newsect.getName());
				return newsect.VirtualAddress;
			} else if ((current > sect.VirtualAddress)
					&& (current < (sect.VirtualAddress + sect.VirtualSize))) {
				long diff = current - sect.VirtualAddress;
				PESection newsect = (PESection) newsections.get(i);
				// System.out.println("Translation VA found INSIDE " + current +
				// " = " + i + " (" +newsect.VirtualAddress + ")=" +
				// newsect.getName());
	...
	private long findNewSize(long current, Vector oldsections,
		...
		for (int i = 0; i < oldsections.size(); i++) {
			...
			if (sect.VirtualAddress == current) {
				PESection newsect = (PESection) newsections.get(i);
				// System.out.println("Translation Size found for " + current +
				// " = " + i + " (" +newsect.VirtualAddress + ")=" +
				// newsect.getName());
				// System.out.println("         Old size " + sect.VirtualSize +
				// " vs new size " + newsect.VirtualSize);
