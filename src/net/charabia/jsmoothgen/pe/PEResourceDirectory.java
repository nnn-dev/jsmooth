...
public class PEResourceDirectory {
	...
	/*
	 * typedef struct IMAGE_RESOURCE_DIRECTORY { uint32_t Characteristics;
	 * uint32_t TimeDateStamp; uint16_t MajorVersion; uint16_t MinorVersion;
	...
	public class DataEntry {
		...
		public int buildBuffer(ByteBuffer buffer, long virtualBaseOffset,
				int dataOffset) {
			// System.out.println("Building Data Entry buffer @ " +
	...
	public class ResourceEntry {
		...
		public String extractStringAt(FileChannel chan, int offset)
				throws IOException {
			long orgchanpos = chan.position();
			chan.position(PEResourceDirectory.this.m_offsetBase + offset);
		...
		public int buildBuffer(ByteBuffer buffer, long virtualBaseOffset,
				int dataOffset) {
			// System.out.println("Building Resource Entry buffer  " + Name +
			// "/" + Id + " @ " + buffer.position() + " (" + dataOffset + ")");
			if (Name != null) {
	...
	public class ImageResourceDirectory {
		...
		public int buildBuffer(ByteBuffer buffer, long virtualBaseOffset) {
			// System.out.println("Building Directory Entry buffer @ " +
			// buffer.position());
		...
		public ResourceEntry getResourceEntry(String name) {
			...
			if (name == null) {
				if (NamedEntries.size() > 0) {
					return (PEResourceDirectory.ResourceEntry) NamedEntries
							.get(0);
				}
				if (IdEntries.size() > 0) {
	...
	public ByteBuffer buildResource(long virtualBaseOffset) {
		// System.out.println("BUILDING RESOURCE / VIRTUAL: " +
		// virtualBaseOffset);
	...
	public boolean replaceResource(String catId, int resourceId, int langId,
		...
		if ((catEntry != null) && (catEntry.Directory != null)) {
			ResourceEntry identEntry = catEntry.Directory
					.getResourceEntry(resourceId);
			if ((identEntry != null) && (identEntry.Directory != null)) {
				ResourceEntry langEntry = identEntry.Directory
						.getResourceEntry(langId);
