...
public class IcoCodec {
	...
	static public class IconHeader {
		...
		public long ColorsImportant; /*
									 * Minimum number of important colors DWORD
									 * 36
									 */
	...
	static public BufferedImage[] loadImages(File f) throws IOException {
		...
		try {
			...
			for (int i = 0; i < dir.idCount; i++) {
				...
				BufferedImage image = new BufferedImage((int) header.Width,
						(int) header.Height / 2, BufferedImage.TYPE_INT_ARGB);
	...
	static private void loadPalettedImage(BinaryInputStream in,
			IconEntry entry, IconHeader header, BufferedImage image)
			throws Exception {
		...
		for (int y = (int) (header.Height / 2) - 1; y >= 0; y--) {
			for (int x = 0; x < header.Width; x++) {
				...
				case 4: {
					...
					int col1 = (pix >> 4) & 0x0F;
					int col2 = pix & 0x0F;
					image.setRGB(x, y, (0xFF << 24) | (redp[col1] << 16)
							| (greenp[col1] << 8) | bluep[col1]);
					image.setRGB(++x, y, (0xFF << 24) | (redp[col2] << 16)
							| (greenp[col2] << 8) | bluep[col2]);
				}
					break;
				case 8: {
